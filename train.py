...
def training(dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wandb=None, logger=None, ply_path=None):
    ...
    for iteration in range(first_iter, opt.iterations + 1):    
        ...
        if args.use_prefilter:
            ...
            voxel_visible_mask = torch.ones(gaussians.get_anchor.shape[0], dtype=torch.bool, device="cuda")
        # print(voxel_visible_mask.shape)

        retain_grad = (iteration < opt.update_until and iteration >= 0)
        render_pkg = render(viewpoint_cam, gaussians, pipe, background, visible_mask=voxel_visible_mask, retain_grad=retain_grad, return_depth=False, distortion_opacity=args.distortion_opacity)
        
        image, viewspace_point_tensor, visibility_filter, offset_selection_mask, radii, scaling, opacity = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["selection_mask"], render_pkg["radii"], render_pkg["scaling"], render_pkg["neural_opacity"]

        lat, lon = render_pkg["lat"], render_pkg["lon"]

        if args.dataset_type == '360Roam' and args.crop_image:
                image[:, 959:, :] = 0

        gt_image = viewpoint_cam.original_image.cuda()

        Ll1 = l1_loss(image, gt_image, mean=False)
        ssim_loss = (1.0 - ssim(image, gt_image, mean=False))
        scaling_reg = scaling.prod(dim=1).mean()

        H, W = gt_image.shape[1], gt_image.shape[2]
        device = gt_image.device
        y_coords = torch.linspace(0, H - 1, H, device=device)
        theta = (y_coords / (H - 1)) * torch.pi - (torch.pi / 2.0)
        weights_1d = torch.cos(theta)
        weights = weights_1d.view(1, H, 1).expand(1, H, W)

        loss_color = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * ssim_loss
        loss = (loss_color * weights).sum() / weights.sum()
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                progress_bar.set_postfix({
                    ...
                    "SSIM": f"{(1 - ssim_loss).mean():.{7}f}",     
                    "PSNR": f"{psnr(image, gt_image).mean():.{7}f}" 
                })
            ...
            # Log and save
            training_report(tb_writer, dataset_name, iteration, Ll1.mean(), loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background), wandb, logger)
            ...
            if iteration < opt.update_until and iteration > opt.start_stat:
                # add statis
                gaussians.training_statis(viewspace_point_tensor, opacity, visibility_filter, offset_selection_mask, voxel_visible_mask, lat)
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        render_pkg = render(view, gaussians, pipeline, background, visible_mask=voxel_visible_mask, distortion_opacity=args.distortion_opacity)
...
def evaluate(model_paths, dataset_type, crop_image, visible_count=None, wandb=None, tb_writer=None, dataset_name=None, logger=None):
    ...
    for method in os.listdir(test_dir):
        ...
        if dataset_type == '360Roam' and crop_image:
            renders = [img[:, :, :959, :] for img in renders]
            gts = [img[:, :, :959, :] for img in gts]
...
if __name__ == "__main__":
    ...
    # calc metrics
    logger.info("\n Starting evaluation...")
    evaluate(args.model_path, args.dataset_type, args.crop_image, visible_count=visible_count, wandb=wandb, logger=logger)
    logger.info("\nEvaluating complete.")
