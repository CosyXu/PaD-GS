...
def readCamerasFromOpenMVG(path, extrinsicsfile, cam_dict, white_background, dataset_type='360Roam', crop_image=True):
    cam_infos = []

    if dataset_type == '360Roam' and crop_image:
        print('###CROP DOWNSIDE IMAGE###')
    ...
    with open(os.path.join(path, extrinsicsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            # print(R, T, np.array(frame["value"]["center"]))
...
def readOpenMVGInfo(path, white_background, eval, dataset_type='360Roam', crop_image=True):
    ...
    cam_infos_unsorted = readCamerasFromOpenMVG(path, "data_extrinsics.json", camfile_dict, white_background, dataset_type, crop_image)
